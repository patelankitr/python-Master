...
def ensure_screenshot_folder_exists(folder):
    ...
    screenshot_dir = os.path.join("/screenshot", folder)
    os.makedirs(screenshot_dir, exist_ok=True)
    return screenshot_dir


def take_element_screenshot(element, screenshot_name, folder="dynamic"):
...
def compare_element_images(dynamic_image_name, baseline_folder="baseline", dynamic_folder="dynamic"):
    ...
    baseline_image_path = os.path.join("/screenshot", baseline_folder, dynamic_image_name)
    print(f"baseline_image_path: {baseline_image_path}")
    dynamic_image_path = os.path.join("/screenshot", dynamic_folder, dynamic_image_name)
    ...
    if diff.getbbox() is None:
        ...
        diff_path = os.path.join("/screenshot", dynamic_folder, f"{dynamic_image_name}_diff.png")
        diff.save(diff_path)
